...
class SdkTemplatePullTests(AdmsTestCase):
    ...
    Fingerprints were only ever read off an `att` terminal by the manual
    "Sync Templates" action: `pull_device` (Sync All, and the auto-pull on
    approval) fetched people and punches and stopped there. A site that
    enrolled every finger on the terminal itself, then synced, ended up with
    a full employee list and an empty `fingerprint_templates` table, and the
    employee drawer said "No templates stored" about people who verify by
    finger every day. Found on 2145222460344: 72 templates on the device, 0
    in the DB.
    """
    ...
    class _FakeUser:
        def __init__(self, uid, user_id, name=""):
            ...
            self.privilege, self.card, self.password, self.group_id = 0, 0, "", ""

    class _FakeFinger:
        def __init__(self, uid, fid, valid=1, blob=b"\x01\x02"):
            self.uid, self.fid, self.valid, self.size = uid, fid, valid, len(blob)
            self._blob = blob
        def json_pack(self):
    ...
    def setUp(self):
        ...
        self.users = [self._FakeUser(1, "3"), self._FakeUser(2, "14")]
        # The shape seen in the field: right index + right middle for most,
        # a third finger for some.
        self.fingers = [
            self._FakeFinger(1, 6), self._FakeFinger(1, 7), self._FakeFinger(1, 8),
            self._FakeFinger(2, 6), self._FakeFinger(2, 7),
        ]

        self._orig_session = self.poller.SessionLocal
        self._orig_connect = self.poller._connect
        self.poller.SessionLocal = self.Session
        self.poller._connect = lambda device: self._FakeConn(self.users, self.fingers)
    ...
    def rows(self):
        db = self.Session()
        try:
            return db.query(FingerprintTemplate).order_by(
                FingerprintTemplate.user_id, FingerprintTemplate.finger_id
            ).all()
        finally:
            db.close()

    def test_a_pull_stores_one_row_per_finger_keyed_on_user_id(self):
        result = self.poller.pull_templates(self.SN)
        self.assertEqual(result["templates_synced"], 5)
        self.assertEqual(result["errors"], [])
        rows = self.rows()
        self.assertEqual(
            [(r.user_id, r.finger_id, r.valid, r.source_device_sn) for r in rows],
            [("14", 6, 1, self.SN), ("14", 7, 1, self.SN),
             ("3", 6, 1, self.SN), ("3", 7, 1, self.SN), ("3", 8, 1, self.SN)],
        )

    def test_a_second_pull_updates_in_place_and_never_duplicates(self):
        self.poller.pull_templates(self.SN)
        self.fingers[0] = self._FakeFinger(1, 6, blob=b"\xff\xfe\xfd")
        result = self.poller.pull_templates(self.SN)
        self.assertEqual(result["templates_synced"], 5)
        rows = self.rows()
        self.assertEqual(len(rows), 5)
        self.assertEqual(
            next(r for r in rows if r.user_id == "3" and r.finger_id == 6).template,
            "fffefd",
        )

    def test_a_finger_whose_uid_is_unknown_is_skipped_not_misfiled(self):
        self.fingers.append(self._FakeFinger(99, 6))
        result = self.poller.pull_templates(self.SN)
        self.assertEqual(result["templates_synced"], 5)
        self.assertEqual(len(self.rows()), 5)

    def test_sync_all_now_pulls_templates_after_people(self):
        """The regression: Sync All alone must leave the drawer populated."""
        result = self.poller.pull_device(self.SN)
        self.assertEqual(result["users_synced"], 2)
        self.assertEqual(result["templates_synced"], 5)
        self.assertEqual(result["errors"], [])
        self.assertEqual(len(self.rows()), 5)
    ...
    def test_a_connection_failure_is_reported_not_raised(self):
        ...
        def _refuse(device):
            raise ZKNetworkError("can't reach device (ping 192.0.2.60)")
        self.poller._connect = _refuse
        result = self.poller.pull_templates(self.SN)
        self.assertEqual(result["templates_synced"], 0)
